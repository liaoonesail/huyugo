...
<body>
	...
	<div class="publicBox ListBox">
		...
		<div class="ListDetails">
			<ul class="clearfix">
				<%
					List<Temai> list = (List<Temai>)request.getAttribute("list");
					for(Temai g : list){
				%>
				<li><a
					href="/huyugo/goods/showGoodsById.do?id=<%=g.getGoods_id() %>&status=temai">
						<img id="imgtemai<%=g.getGoods_id() %>" />
						<p id="ptemai<%=g.getGoods_id() %>"></p>
						<dt>
							<span></span><span></span>
						</dt>
						<dd>
							<span>¥<%=g.getReal_price() %></span>
							<del id="deltemai<%=g.getGoods_id() %>"></del>
							<em>马上抢</em>
						</dd> </a></li>
						<script>getGoodsData("<%=g.getGoods_id() %>","temai");</script>
				<%
					}
				 %>
