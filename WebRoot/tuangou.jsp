...
<body>
	...
	<div class="publicBox ListBox">
		...
		<div class="ListDetails">
			<ul class="clearfix">
				<%
					List<Tuangou> list = (List<Tuangou>)request.getAttribute("list");
							for(Tuangou g : list){
				%>
				<li><a
					href="/huyugo/goods/showGoodsById.do?id=<%=g.getGoods_id()%>&status=tuangou">
						<img id="imgtuangou<%=g.getGoods_id()%>" />
						<p id="ptuangou<%=g.getGoods_id()%>"></p>
						<dt>
							<span></span><span></span>
						</dt>
						<dd>
							<span>¥<%=g.getReal_price()%></span>
							<del id="deltuangou<%=g.getGoods_id()%>"></del>
							<em>马上抢</em>
						</dd> </a>
				</li>
				<script>getGoodsData("<%=g.getGoods_id()%>", "tuangou");
				</script>
				<%
					}
				%>
