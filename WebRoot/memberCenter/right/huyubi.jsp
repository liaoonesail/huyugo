		...
		<div class="publicBox2">
			...
			<table width="100%" style="border-left: 1px solid #F5F5F5;border-right: 1px solid #F5F5F5;">
				...
				for(Receipt_Payment payment : (List<Receipt_Payment>)paymentList.get("list")){
				 ...
				 <tr>
				<td></td>
				<td><a href="/huyugo/payment/showHuyubi.do?curpage=<% if (curr == 1) %><%=curpage %><% else %><%=curr-1%>">上一页</a></td>
				<td><a href="/huyugo/payment/showHuyubi.do?curpage=<% if (curr == (Integer)paymentList.get("countPage")) %><%=curpage %><% else %><%=curr+1%>">下一页</a></td>
				<td></td>
			</tr>
			</table>
		</div>
