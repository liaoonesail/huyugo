...
<body>
	...
	<div class="publicBox ListBox">
		...
		<div class="actricNva clearfix">
			...
			<p>></p>
			<p class="type">VIP商城</p>
		</div>
		<!--分类商品列表-->
		<div class="ListDetails">
			<ul class="clearfix">
				<%
					List<Vip_shop> cuxiaoList = (List<Vip_shop>)request.getAttribute("vipshopList");
							for(Vip_shop cuxiao : cuxiaoList){
				%>
				<li><a
					href="/huyugo/goods/showGoodsById.do?id=<%=cuxiao.getGoods_id()%>&status=vipshop">
						<img id="imgvipshop<%=cuxiao.getGoods_id()%>" />
						<p class="p1" id="pvipshop<%=cuxiao.getGoods_id()%>"></p>
						<dt>
							<span></span><span></span>
						</dt>
						<dd>
							<span>￥<%=cuxiao.getReal_price()%></span>
							<%--<del id="delvipshop<%=cuxiao.getGoods_id()%>"></del>
							<em>马上抢</em>--%>
						</dd> </a>
				</li>
				<script>getGoodsData("<%=cuxiao.getGoods_id()%>", "vipshop");
				</script>
				<%
					}
				%>
